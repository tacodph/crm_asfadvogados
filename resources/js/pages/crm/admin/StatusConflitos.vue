...
import {
    ...
    edit,
    store,
} from '@/actions/App/Http/Controllers/Admin/StatusConflitoController';
...
const form = useForm({
    ...
    slug: '',
    cor_fundo: '#E7F0EE',
    cor_texto: 'var(--accent)',
    cor_fundo_detalhe: '#F1F6F4',
    cor_borda_detalhe: 'color-mix(in srgb, var(--accent) 35%, transparent)',
    ordem: 0,
});
...
const submit = (): void => {
    form.post(store.url(), {
        ...
        onSuccess: () => {
            ...
            form.cor_fundo = '#E7F0EE';
            form.cor_texto = 'var(--accent)';
            form.cor_fundo_detalhe = '#F1F6F4';
            form.cor_borda_detalhe = 'color-mix(in srgb, var(--accent) 35%, transparent)';
            form.ordem = 0;
        },
    });
};
...
const remove = (id: number): void => {
    ...
    router.delete(destroy.url({ status_conflito: id }), { preserveScroll: true });
};
</script>
    ...
    <div class="mx-auto flex max-w-[980px] flex-col gap-4">
        ...
        >
            ...
            <input
                ...
                :class="fieldClass"
            />
            <input v-model="form.cor_fundo" type="color" title="Fundo" :class="fieldClass" />
            <input v-model="form.cor_texto" type="color" title="Texto" :class="fieldClass" />
                    ...
                    >
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-2">
                                <Link
                                    :href="
                                        edit.url({ status_conflito: item.id })
                                    "
